...
export default function Home() {
  ...
  const selectedBrandOpenRequirements = selectedBrandRequirements.filter((requirement) => getRequirementCoverage(requirement).gap > 0);
  const selectedBrandCoveredRequirements = selectedBrandRequirements.filter((requirement) => getRequirementCoverage(requirement).gap === 0);
      ...
      {selectedBrand && selectedBrandSummary && <div className="modal-backdrop" role="presentation" onMouseDown={closeBrand}>
        <section className="entity-modal" role="dialog" aria-modal="true" aria-labelledby="brand-modal-title" onMouseDown={(event) => event.stopPropagation()}>
          ...
          <div className="modal-section"><div><span className="kicker">REQUISITOS DE {selectedBrand.name.toUpperCase()}</span><h3>Requisitos de certificación</h3></div>
            {selectedBrandRequirements.length > 0 ? <div className="requirement-groups">
              <div className="requirement-group"><div className="requirement-group-heading"><strong>No cubiertos</strong><span>{selectedBrandOpenRequirements.length}</span></div>{selectedBrandOpenRequirements.length > 0 ? <div className="brand-requirements">{selectedBrandOpenRequirements.map((requirement) => <button type="button" key={requirement.id} onClick={() => openRequirement(requirement.id)}><strong>{certificationById.get(requirement.certification_id)?.name ?? "Certificación sin identificar"}</strong><span>{getRequirementCoverage(requirement).achieved} de {getRequirementCoverage(requirement).required} cubiertos · {getRequirementCoverage(requirement).compliance}%</span></button>)}</div> : <p className="modal-empty">Todos los requisitos están cubiertos.</p>}</div>
              <div className="requirement-group"><div className="requirement-group-heading"><strong>Cubiertos</strong><span>{selectedBrandCoveredRequirements.length}</span></div>{selectedBrandCoveredRequirements.length > 0 ? <div className="brand-requirements">{selectedBrandCoveredRequirements.map((requirement) => <button type="button" key={requirement.id} onClick={() => openRequirement(requirement.id)}><strong>{certificationById.get(requirement.certification_id)?.name ?? "Certificación sin identificar"}</strong><span>{getRequirementCoverage(requirement).achieved} de {getRequirementCoverage(requirement).required} cubiertos · {getRequirementCoverage(requirement).compliance}%</span></button>)}</div> : <p className="modal-empty">Aún no hay requisitos cubiertos.</p>}</div>
