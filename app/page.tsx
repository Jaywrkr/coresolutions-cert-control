    ...
    <main className="shell">
      ...
      {selectedRequirement && <div className="modal-backdrop record-detail-backdrop" role="presentation" onMouseDown={closeAllModals}>
        <section className="entity-modal record-detail-modal" role="dialog" aria-modal="true" aria-labelledby="requirement-modal-title" onMouseDown={(event) => event.stopPropagation()}>
          ...
          {canManage && records.filter((record) => record.certification_id === selectedRequirement.certification_id).length > 0 && <div className="modal-section assignment-management"><div><span className="kicker">ADMINISTRAR</span><h3>Asignaciones registradas</h3></div><div className="assignment-delete-list">{records.filter((record) => record.certification_id === selectedRequirement.certification_id).map((record) => <div key={record.id}><span>{technicianNameById.get(record.technician_id) ?? "Técnico sin identificar"}</span><button type="button" className="danger-action" onClick={() => deleteTechnicianCertification(record)}>Eliminar</button></div>)}</div></div>}
